...
describe("POST /fuelquote", () => {
  ...
  it("Should return 400 if gallons request is not numeric", async () => {
    const response = await testSession.post("/fuelquote").send({
      gallonsRequested: "fifty",
      deliveryAddress: "1900 Coco Drive. Houston, TX 77882",
      deliveryDate: "2022-04-10",
      suggestedPrice: 3.14,
    });
  ...
  if (
    ("Should return 400 if the delivery date is not a valid date in yyyy-mm-dd format",
    async () => {
      const response = await testSession.post("/fuelquote").send({
        gallonsRequested: "50",
        deliveryAddress: "1900 Coco Drive. Houston, TX 77882",
      ...
      expect(response.text).toBe(
        "invalid date format, date is not in yyyy-mm-dd form"
      );
    })
  )
